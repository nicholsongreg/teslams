...
var argv = require('optimist')
    ...
    .default('S', 1)
    .alias('o', 'owntracks')
    .describe('o', 'Owntracks Logging to MQTT server (appended to owntracks/)')
    .alias('T', 'owntid')
    .describe('T', 'Owntracks Tid (appended to owntracks/')
    .alias('v', 'values')
...
if (process.env.MQTT) { argv.mqtt = process.env.MQTT };
if (process.env.OWNTRACKS) { argv.owntracks = process.env.OWNTRACKS };
...
if (argv.owntracks) {
    console.log("Owntracks to owntracks/" + argv.owntracks + " as TID:" + argv.owntid);
}
/**
        ...
        if ((argv.mqtt || argv.awsiot) && argv.topic) {
            ...
            if (argv.owntracks) {
                var otdata = {
                    ...
                    conn: "m",
                    doze: false,
                    lat: streamdata.est_lat,
                    lon: streamdata.est_lon,
                }
                if (!argv.silent) { console.log('Publishing to topic = owntracks/' + argv.owntracks + ' as ' + argv.owntid + '\n streamdata = ' + JSON.stringify(otdata)); }

                try {
                    client.publish('owntracks/' + argv.owntracks, JSON.stringify(otdata));
                }
                catch (error) {
            ...
            if (argv.mqtt) {
                for (const [key, value] of Object.entries(streamdata)) {
                    try {
                        client.publish(argv.owntracks + '/' + key, value);
                        if (!argv.silent) { console.log('Published to topic = ' + argv.owntracks + '/' + key + '\n streamdata = ' + value); }
                    } catch (error) {
        ...
        teslams.get_charge_state(getAux.vid, function (data) {
            ...
            if (argv.mqtt) {
                for (const [key, value] of Object.entries(data)) {
                    try {
                        client.publish(argv.owntracks + '/' + key, value.toString());
                        console.log('Published to topic = ' + argv.owntracks + '/' + key + '\tdata = ' + value.toString());
                    } catch (error) {
                        // failed to send, therefore stop publishing and log the error thrown
                        console.log('Error while publishing message to mqtt broker: ' + error.toString());
                        console.log('	Failed to topic = ' + argv.owntracks + '/' + key + '\n streamdata = ' + value);
                ...
                // try {
                // 	client.publish(argv.owntracks + '/' + getAux.vid + '/charge_state', JSON.stringify(data));
                // } catch (error) {
                // 	// failed to send, therefore stop publishing and log the error thrown
